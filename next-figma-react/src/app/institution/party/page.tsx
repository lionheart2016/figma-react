...
import {
  ...
  FormErrors,
  KeyPartiesFormProps
} from '@/types/institutional-auth';

const KeyPartiesForm: React.FC<KeyPartiesFormProps> = ({ onSubmit, initialData }) => {
   const { t } = useTranslation();
   const { isDarkMode } = useTheme();
   
   const [formData, setFormData] = useState<KeyPartiesFormData>({
     parties: initialData?.parties || [],
     currentPartyType: initialData?.currentPartyType || 'individual'
   });
  ...
  const validateParty = (party: Partial<Party>, type: PartyType): FormErrors => {
  ...
  const addParty = () => {
    const errors = validateParty(newParty, formData.currentPartyType);
    
    if (Object.keys(errors).length === 0) {
      const updatedFormData = {
        ...formData,
        parties: [...formData.parties, newParty as Party]
      };
...
const steps: StepData[] = [
  { id: 'info', title: '机构信息', status: 'completed' },
  { id: 'address', title: '地址信息', status: 'completed' },
  { id: 'party', title: '相关方信息', status: 'current' },
 ...
 * 机构认证流程的第三步 - 收集关键方信息
 */
const KeyPartiesPage: React.FC = () => {
  const router = useRouter();
  const { t } = useTranslation();

  // 处理表单提交
  const handleFormSubmit = (formData: KeyPartiesFormData) => {
    console.log('关键方信息表单提交:', formData);
    
    // 跳转到下一步（文件上传页面）
    router.push('/institution/document');
  };
  ...
  return (
    <Layout 
      ...
      subtitle="Key Parties Information"
    >
      <KeyPartiesForm 
