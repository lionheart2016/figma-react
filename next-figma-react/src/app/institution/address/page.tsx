...
import { StepData, AddressInfoFormData, FormErrors, AddressInfoFormProps } from '@/types/institutional-auth';

const AddressInfoForm: React.FC<AddressInfoFormProps> = ({ onSubmit, initialData = {} }) => {
  ...
  const [formData, setFormData] = useState<AddressInfoFormData>({
    ...
    isSameAsRegistered: initialData.isSameAsRegistered ?? true,
...
const steps: StepData[] = [
  ...
  { id: 'address', title: '地址信息', status: 'current' },
...
export default function AddressInfoPage() {
  const handleFormSubmit = (data: AddressInfoFormData) => {
    console.log('地址信息表单提交:', data);
    // 这里可以添加表单提交逻辑，如API调用等
    // 提交成功后跳转到下一步
    window.location.href = '/institution/party';
  };
  ...
  return (
    ...
    >
      <AddressInfoForm onSubmit={handleFormSubmit} />
    </Layout>
  );
}