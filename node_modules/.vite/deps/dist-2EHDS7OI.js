...
import {
  ...
  validate,
  waitForCallsStatus
} from "./chunk-X7CSQFPI.js";
import "./chunk-TAIEUFJI.js";
import {
  BaseError,
  BaseError2,
  ...
  toHex,
  trim
} from "./chunk-AI6JS3BP.js";
import {
  sha256,
  sha384,
  sha512
} from "./chunk-7NXX7H3F.js";
import "./chunk-FKJ57UY7.js";
import {
  abytes,
  bytesToHex,
  concatBytes,
  randomBytes
} from "./chunk-2D7FYFAH.js";
import {
  __export
...
var SHA256 = class extends HashMD {
  ...
  process(view, offset) {
    ...
    for (let i4 = 0; i4 < 64; i4++) {
      ...
      const T12 = H + sigma1 + Chi(E, F2, G) + SHA256_K[i4] + SHA256_W[i4] | 0;
...
var sha3842 = createHasher3(() => new SHA384());
...
function isEqual(hexA, hexB) {
  return equalBytes2(fromHex4(hexA), fromHex4(hexB));
}
...
function checksum2(address) {
  ...
  for (let i4 = 0; i4 < 40; i4 += 2) {
    if (hash9[i4 >> 1] >> 4 >= 8 && characters[i4]) {
      characters[i4] = characters[i4].toUpperCase();
    }
    if ((hash9[i4 >> 1] & 15) >= 8 && characters[i4 + 1]) {
...
(function(encodePacked3) {
  function encode11(type6, value, isArray = false) {
    ...
    if (arrayMatch && Array.isArray(value)) {
      ...
      const data = [];
      for (let i4 = 0; i4 < value.length; i4++) {
        data.push(encode11(childType, value[i4], true));
      }
      if (data.length === 0)
        return "0x";
      return concat4(...data);
    ...
    if (args_.length > 0) {
      ...
      topics = (indexedInputs == null ? void 0 : indexedInputs.map((param, i4) => {
        if (Array.isArray(args_[i4]))
          return args_[i4].map((_2, j3) => encode11(param, args_[i4][j3]));
        return typeof args_[i4] !== "undefined" && args_[i4] !== null ? encode11(param, args_[i4]) : null;
      })) ?? [];
    }
  }
  const selector = (() => {
...
function fromDerHex(signature) {
  const { r: r3, s: s3 } = secp256k1.Signature.fromDER(from6(signature).slice(2));
  return { r: r3, s: s3 };
}
...
var integerToCharacter2 = Object.fromEntries(Array.from("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/").map((a3, i4) => [i4, a3.charCodeAt(0)]));
...
function fromBytes7(value, options = {}) {
  ...
  for (let i4 = 0, j3 = 0; j3 < value.length; i4 += 4, j3 += 3) {
    ...
    encoded[i4] = integerToCharacter2[y3 >> 18];
    encoded[i4 + 1] = integerToCharacter2[y3 >> 12 & 63];
    encoded[i4 + 2] = integerToCharacter2[y3 >> 6 & 63];
    encoded[i4 + 3] = integerToCharacter2[y3 & 63];
  }
...
function toBytes8(value) {
  ...
  for (let i4 = 0, j3 = 0; i4 < base64.length; i4 += 4, j3 += 3) {
    const x2 = (characterToInteger2[decoded[i4]] << 18) + (characterToInteger2[decoded[i4 + 1]] << 12) + (characterToInteger2[decoded[i4 + 2]] << 6) + characterToInteger2[decoded[i4 + 3]];
    decoded[j3] = x2 >> 16;
...
function toHex9(value) {
  return fromBytes2(toBytes8(value));
}
function toString5(value) {
  return toString3(toBytes8(value));
}
  ...
  function inner(node) {
    ...
    while (level.length > 1) {
      ...
      for (let i4 = 0; i4 < level.length; i4 += 2)
        level_.push(hash3(concat5(level[i4], level[i4 + 1])));
      level = level_;
    }
...
function from21(data, options = {}) {
  ...
  return as === "Bytes" ? blobs.map((x2) => x2.bytes) : blobs.map((x2) => fromBytes2(x2.bytes));
}
...
function to2(blobs, to3) {
  ...
  const blobs_ = typeof blobs[0] === "string" ? blobs.map((x2) => fromHex4(x2)) : blobs;
...
function toCommitments(blobs, options) {
  ...
  return as === "Bytes" ? commitments : commitments.map((x2) => fromBytes2(x2));
}
function toProofs(blobs, options) {
  const { kzg } = options;
  const as = options.as ?? (typeof blobs[0] === "string" ? "Hex" : "Bytes");
  const blobs_ = typeof blobs[0] === "string" ? blobs.map((x2) => fromHex4(x2)) : blobs;
  const commitments = typeof options.commitments[0] === "string" ? options.commitments.map((x2) => fromHex4(x2)) : options.commitments;
  const proofs = [];
  ...
  return as === "Bytes" ? proofs : proofs.map((x2) => fromBytes2(x2));
}
...
function contains(bloom, input) {
  ...
  for (const i4 of [0, 2, 4]) {
    const bit = hash9[i4 + 1] + (hash9[i4] << 8) & 2047;
...
function createBlsPairing(fields, G1, G2, params) {
  ...
  function millerLoopBatch(pairs, withFinalExponent = false) {
    ...
    if (pairs.length) {
      ...
      for (let i4 = 0; i4 < ellLen; i4++) {
        f12 = Fp122.sqr(f12);
        for (const [ell, Px, Py] of pairs) {
          for (const [c0, c1, c22] of ell[i4])
...
function createBlsSig(blsPairing, PubCurve, SigCurve, SignatureCoder, isSigG1) {
  ...
  return {
    ...
    verifyBatch(signature, messages, publicKeys) {
      ...
      for (let i4 = 0; i4 < nPublicKeys.length; i4++) {
        const pub = nPublicKeys[i4];
        const msg = nMessages[i4];
...
function psiFrobenius(Fp3, Fp22, base2) {
  const PSI_X = Fp22.pow(base2, (Fp3.ORDER - _1n3) / _3n2);
  const PSI_Y = Fp22.pow(base2, (Fp3.ORDER - _1n3) / _2n3);
  function psi(x2, y3) {
    const x22 = Fp22.mul(Fp22.frobeniusMap(x2, 1), PSI_X);
    const y22 = Fp22.mul(Fp22.frobeniusMap(y3, 1), PSI_Y);
    return [x22, y22];
  }
  const PSI2_X = Fp22.pow(base2, (Fp3.ORDER ** _2n3 - _1n3) / _3n2);
  const PSI2_Y = Fp22.pow(base2, (Fp3.ORDER ** _2n3 - _1n3) / _2n3);
...
var _Field12 = class {
  ...
  _cyclotomicExp(num, n2) {
    let z3 = this.ONE;
    for (let i4 = this.X_LEN - 1; i4 >= 0; i4--) {
...
].map((i4) => i4.map((pair) => Fp2.fromBigTuple(pair.map(BigInt)))));
...
function toHex11(point) {
  return fromBytes2(toBytes10(point));
}
...
function unsafe_atob(s3) {
  ...
  for (let i4 = 0, pos = 0, width = 0, carry = 0; i4 < n2; i4++) {
...
function read_deltas(n2, next) {
  let v3 = Array(n2);
  for (let i4 = 0, x2 = 0; i4 < n2; i4++) v3[i4] = x2 += signed(next());
  return v3;
}
...
function read_linear_table(w3, next) {
  ...
  return m2.flatMap((v3, i4) => {
    let [x2, ...ys] = v3;
    return Array(vN[i4]).fill().map((_2, j3) => {
      let j_dy = j3 * dy;
...
function compare_arrays(a3, b2) {
  let n2 = a3.length;
  let c3 = n2 - b2.length;
  for (let i4 = 0; c3 == 0 && i4 < n2; i4++) c3 = a3[i4] - b2[i4];
  return c3;
}
...
function init$1() {
  ...
  SHIFTED_RANK = new Map(read_sorted_arrays(r3).flatMap((v3, i4) => v3.map((x2) => [x2, i4 + 1 << 24])));
...
function init() {
  ...
  const set_add_many = (set2, v3) => v3.forEach((x2) => set2.add(x2));
  ...
  GROUPS = read_array_while((i4) => {
    let N2 = read_array_while(r3).map((x2) => x2 + 96);
    if (N2.length) {
  ...
  wholes.forEach((cp, i4) => {
    let d3 = r3();
    let w3 = wholes[i4] = d3 ? wholes[i4 - d3] : { V: [], M: /* @__PURE__ */ new Map() };
    w3.V.push(cp);
  ...
  EMOJI_LIST = read_trie(r3).map((v3) => Emoji.from(v3)).sort(compare_arrays);
  ...
  if (g2.M) {
    ...
    for (let i4 = 1, e3 = decomposed2.length; i4 < e3; i4++) {
      if (NSM.has(decomposed2[i4])) {
        ...
        if (j3 - i4 > NSM_MAX) {
          throw new Error(`excessive non-spacing marks: ${bidi_qq(safe_str_from_cps(decomposed2.slice(i4 - 1, j3)))} (${j3 - i4}/${NSM_MAX})`);
        }
        i4 = j3;
      }
    }
...
function namehash(name) {
  ...
  for (let i4 = labels.length - 1; i4 >= 0; i4 -= 1) {
    ...
    const hashed = hashFromEncodedLabel ? fromHex4(hashFromEncodedLabel) : keccak2563(fromString4(labels[i4]), { as: "Bytes" });
...
function sign3(options) {
  ...
  const { r: r3, s: s3, recovery } = secp256k1.sign(from7(payload), from7(privateKey), {
...
var p256_hasher2 = (() => {
  return createHasher2(p2563.Point, createSWU(p2563.Point, {
    A: p256_CURVE.a,
    B: p256_CURVE.b,
...
var p384_hasher2 = (() => {
  return createHasher2(p3842.Point, createSWU(p3842.Point, {
    A: p384_CURVE.a,
    B: p384_CURVE.b,
...
var p521_hasher2 = (() => {
  return createHasher2(p5212.Point, createSWU(p5212.Point, {
    A: p521_CURVE.a,
    B: p521_CURVE.b,
...
function recoverPublicKey2(options) {
  const { payload, signature } = options;
  const { r: r3, s: s3, yParity } = signature;
  const signature_ = new secp256r13.Signature(BigInt(r3), BigInt(s3)).addRecoveryBit(yParity);
...
function sign4(options) {
  const { extraEntropy: extraEntropy3 = extraEntropy2, hash: hash9, payload, privateKey } = options;
  const { r: r3, s: s3, recovery } = secp256r13.sign(payload instanceof Uint8Array ? payload : fromHex4(payload), privateKey instanceof Uint8Array ? privateKey : fromHex4(privateKey), {
    extraEntropy: typeof extraEntropy3 === "boolean" ? extraEntropy3 : from6(extraEntropy3).slice(2),
    lowS: true,
    ...hash9 ? { prehash: true } : {}
  });
...
function encodeField2(properties) {
  ...
  if (type6.lastIndexOf("]") === type6.length - 1) {
    ...
    return [
      { type: "bytes32" },
      keccak2563(encode4(typeValuePairs.map(([t3]) => t3), typeValuePairs.map(([, v3]) => v3)))
    ];
  }
  return [{ type: type6 }, value];
}
...
function parseAsn1Signature2(bytes) {
  ...
  const r3 = BigInt(fromBytes2(bytes.slice(r_start, r_end)));
  const s3 = BigInt(fromBytes2(bytes.slice(s_start)));
  return {
    r: r3,
    s: s3 > p2564.CURVE.n / 2n ? p2564.CURVE.n - s3 : s3
  };
}
...
function getClientDataJSON(options) {
  ...
  return JSON.stringify({
    type: "webauthn.get",
    challenge: fromHex9(challenge, { url: true, pad: false }),
    origin,
    crossOrigin,
    ...extraClientData
  });
...
function getCredentialCreationOptions(options) {
  ...
  }, challenge = createChallenge2, excludeCredentialIds, name: name_, rp = {
...
function verify5(options) {
  ...
  if (fromBytes2(toBytes8(challenge_extracted)) !== challenge)
    return false;
...
async function sign6(options) {
  ...
  const r3 = toBigInt2(slice4(signature_bytes, 0, 32));
  let s3 = toBigInt2(slice4(signature_bytes, 32, 64));
  if (s3 > p2564.CURVE.n / 2n)
    s3 = p2564.CURVE.n - s3;
  return { r: r3, s: s3 };
}
...
function toWebAuthnSignature({ webauthn, signature }) {
  const { r: r3, s: s3 } = fromHex7(signature);
  return encodeAbiParameters([
    {
      components: [
        {
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=dist-2EHDS7OI.js.map
